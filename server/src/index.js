...
app.get('/v1/items', async (req, res) => {
  ...
  const items =
      await pgClient.query('SELECT * FROM items WHERE id = $1', [id])
          .catch(e => {
            res.status(500).send(
                `Encountered an internal error when fetching item with ID ${
                    id}`);
          });
...
app.post('/v1/items', async (req, res) => {
  ...
  const item = await pgClient
    ...
    ($1, $2, $3)`,
                   ...
                   .catch(e => {
                     res.status(500).send(
                         'Encountered an internal error when creating an item');
                   });
...
app.put('/v1/items/:id', async (req, res) => {
  ...
  `,
      ...
      .catch(e => {
        res.status(500).send(
            'Encountered an internal error when updating an item');
      });
